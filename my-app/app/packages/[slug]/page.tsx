...
export default function PackageDetailPage() {
  ...
  if (!trip) return <main className="grid min-h-screen place-items-center bg-[#f5f4ef] px-6 text-center"><div><p className="text-sm text-slate-500">Package not found</p><Link href="/packages" className="mt-5 inline-block underline">Back to packages</Link></div></main>;
  ...
  return (
    <main className="min-h-screen bg-[#f5f4ef] text-slate-950">
      <section className="relative min-h-[82vh] overflow-hidden text-white">
        ...
        <nav className="absolute inset-x-0 top-0 z-10 mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-10"><Link href="/" className="font-serif text-2xl">TravelQ.</Link><Link href="/packages" className="text-sm text-white/75 hover:text-white">← All packages</Link></nav>
        <div className="relative z-10 mx-auto flex min-h-[82vh] max-w-7xl items-end px-6 pb-16 pt-32 lg:px-10 lg:pb-24"><motion.div initial="hidden" animate="visible" variants={fadeUp} className="max-w-5xl"><p className="text-xs uppercase tracking-[0.3em] text-white/60">{trip.category} · {trip.duration}</p><h1 className="mt-6 font-serif text-5xl leading-[0.95] tracking-tight sm:text-7xl lg:text-[8rem]">{trip.title}</h1><p className="mt-6 text-lg text-white/75">{trip.destination}</p></motion.div></div>
      </section>

      <section className="mx-auto grid max-w-7xl gap-12 px-6 py-24 lg:grid-cols-[0.7fr_1.3fr] lg:px-10 lg:py-32"><p className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">The journey</p><div><p className="max-w-3xl font-serif text-4xl leading-tight sm:text-6xl">{trip.description}</p><div className="mt-12 grid gap-6 border-y border-slate-900/10 py-7 sm:grid-cols-3"><div><p className="text-xs uppercase tracking-[0.2em] text-slate-400">Duration</p><p className="mt-2 font-medium">{trip.duration}</p></div><div><p className="text-xs uppercase tracking-[0.2em] text-slate-400">Journey</p><p className="mt-2 font-medium">{trip.category}</p></div><div><p className="text-xs uppercase tracking-[0.2em] text-slate-400">Starting from</p><p className="mt-2 font-medium">{trip.price}</p></div></div></div></section>

      <section className="bg-white px-6 py-20 lg:px-10 lg:py-28"><div className="mx-auto max-w-7xl"><div className="grid gap-10 lg:grid-cols-[0.7fr_1.3fr]"><p className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">The plan</p><div className="divide-y divide-slate-200">{trip.itinerary.map(([day, title, text]: string[]) => <div key={day} className="grid gap-5 py-8 first:pt-0 sm:grid-cols-[120px_1fr]"><p className="text-sm text-slate-400">{day}</p><div><h3 className="font-serif text-2xl sm:text-3xl">{title}</h3><p className="mt-3 max-w-xl leading-7 text-slate-600">{text}</p></div></div>)}</div></div></div></section>

      <section className="mx-auto grid max-w-7xl gap-12 px-6 py-24 lg:grid-cols-[0.7fr_1.3fr] lg:px-10 lg:py-32"><p className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">Included</p><div className="grid gap-5 sm:grid-cols-2">{trip.includes.map((item: string, i: number) => <div key={item} className="border-t border-slate-200 pt-5"><span className="text-xs text-slate-400">0{i + 1}</span><p className="mt-8 text-lg">{item}</p></div>)}</div></section>

      <section className="bg-slate-950 px-6 py-24 text-white lg:px-10 lg:py-32"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_1fr] lg:items-end"><div><p className="text-xs uppercase tracking-[0.3em] text-white/40">Ready to plan?</p><h2 className="mt-7 max-w-3xl font-serif text-5xl leading-tight sm:text-7xl">Make this journey yours.</h2></div><div><p className="max-w-lg text-lg leading-8 text-white/60">Tell us you are interested in {trip.title}. The package context will already be attached to your enquiry.</p><Link href={`/enquire?type=package&slug=${slug}`} className="mt-8 inline-flex rounded-full bg-white px-7 py-4 text-sm font-medium text-slate-950">Enquire about this journey <span className="ml-6">↗</span></Link></div></div></section>
    </main>
  );
}
